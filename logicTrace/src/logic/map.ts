...
class Map {
	...
	private merge(): boolean {
		...
		// 周围格子
		let near = mapApi.nearRange(un.posi, 1)
		...
		flag = near
			...
			.some(posi => {
				...
				if (mapApi.getDirection(pa.posi, so.posi) == mapApi.getDirection(un.posi, so.posi)) {
	...
	private findExtendList(posi: mapApi.IPosition): mapApi.IPosition[] {
		let list: mapApi.IPosition[];
		list = mapApi.nearRange(posi, 1)
			.filter(posi => !this.isOut(posi))
			.filter(posi => {
	...
	// 是否坐标超出地图
	private isOut(posi: mapApi.IPosition): boolean {
